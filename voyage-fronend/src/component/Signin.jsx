  ...
  return (
    <div className="min-h-screen bg-dirtWhite flex justify-center items-center">
      <div className="flex flex-col items-center border rounded-md bg-white h-[80vh] w-full max-w-md p-6 ">
        <h2 className="text-3xl font-poppins font-medium mb-6">Welcome Back</h2>

        <button className="flex justify-center items-center border border-black rounded-full h-10 w-full max-w-sm px-4 mb-4 mt-10">
          <span className="font-medium mr-2">Sign in with Google</span>
          <FaGoogle size={32} />
        </button>

        <button onClick={()=>navigate('/signin/email')} className="flex justify-center items-center border border-black rounded-full h-10 w-full max-w-sm px-4 mb-4">
          <span className="font-medium mr-2">Sign in with Email</span>
          <MdOutlineMailOutline size={32} />
        </button>

        <div className="mt-10">
          <span className="font-medium">No account? </span>
          <span onClick={()=>navigate('/signup')} className="text-green-700 font-semibold cursor-pointer">Create one</span>
        </div>
